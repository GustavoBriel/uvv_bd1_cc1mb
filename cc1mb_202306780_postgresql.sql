...
/*Inclui restrições em tabelas específicas para evitar que dados inválidos sejam inseridos." */

--Restrição que não permite o preço da tabela "produtos" ser negativo
ALTER TABLE produtos      ADD CONSTRAINT preco_positivo           CHECK(preco_unitario >= 0);

--Restrição que não permite o preço da tabela "estoques" ser negativo
ALTER TABLE estoques      ADD CONSTRAINT quantidade_positiva      CHECK(quantidade >= 0);

--Restrição que não permite o preço da tabela "pedidos_itens" ser negativo
ALTER TABLE pedidos_itens ADD CONSTRAINT preco_positivo2          CHECK(preco_unitario >= 0);

--Restrição que não permite a quantidade ser negativa na tabela "pedidos_itens"
ALTER TABLE pedidos_itens ADD CONSTRAINT quantidade_positiva2     CHECK(quantidade >= 0);

-- Não permite que a tabela endeco_web e endereco_fisico fiquem vazias simultaneamente
ALTER TABLE lojas         ADD CONSTRAINT endereco_vazio 	      CHECK(endereco_web IS not null OR endereco_fisico IS not null);

--Restrição na tabela "pedidos" para o status ser somente "cancela,completo,aberto,pago,reembolsado e enviado"
ALTER TABLE pedidos       ADD CONSTRAINT status_certo             CHECK(status IN ('CANCELADO, COMPLETO, ABERTO, PAGO, REEMBOLSADO, ENVIADO'));

--Restrição na tabela "envios" para o status ser somente "criado,enviado,transito e entregue"
